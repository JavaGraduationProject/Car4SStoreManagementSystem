...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="cccccc" class="table table-striped table-bordered table-hover">  
 ...
 <% 
    ...
    String url = "fuwuyuyue_list3.jsp?1=1"; 
...
if(request.getParameter("zhuangtai")=="" ||request.getParameter("zhuangtai")==null ){}else{sql=sql+" and zhuangtai like '%"+request.getParameter("zhuangtai")+"%'";}
 ...
 <td><%=map.get("zhuangtai") %></td>
    ...
    <td width="220" align="center"> <% if(map.get("zhuangtai").equals("未完成")){%><a class="btn btn-danger btn" href="fuwudingdan_add.jsp?id=<%=map.get("id")%>"> <i class="glyphicon glyphicon-pencil" > </i> 开始服务</a><%}%>     <a class="btn btn-info btn-small" href="fuwuyuyue_detail.jsp?id=<%=map.get("id")%>"> <i class="glyphicon glyphicon-random" > </i> 详细</a> </td>
  </tr>
  	<%
  }
   %>
