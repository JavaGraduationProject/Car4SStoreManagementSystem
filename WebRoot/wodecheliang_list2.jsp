...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="cccccc" class="table table-striped table-bordered table-hover">  
  <tr>
    ...
    <td bgcolor='#cccccc'>车牌号</td>
    <td bgcolor='#cccccc'>品牌</td>
    <td bgcolor='#cccccc'>车辆名称</td>
    <td bgcolor='#cccccc'>车型</td>
    <td bgcolor='#cccccc'>驱动方式</td>
    <td bgcolor='#cccccc'>排量</td>
    <td bgcolor='#cccccc'>换挡方式</td>
    <td bgcolor='#cccccc' width='90' align='center'>图片</td>
    <td bgcolor='#cccccc'>总里程</td>
    <td bgcolor='#cccccc'>电压</td>
    <td bgcolor='#cccccc'>机油</td>
    <td bgcolor='#cccccc'>用户名</td>
    <td bgcolor='#cccccc'>姓名</td>
    <td bgcolor='#cccccc'>联系电话</td>
    <td bgcolor='#cccccc'>检查提醒</td>
...
int zonglichengtx=0;
    ...
    String url = "wodecheliang_list2.jsp?1=1"; 
    String sql =  "select * from wodecheliang where yonghuming='"+request.getSession().getAttribute("username")+"' ";
...
if(map.get("jianchatixing").equals("是")){zonglichengtx=zonglichengtx+1;}
 ...
 <td width='90'><a class="btn btn-info btn-small" href='<%=map.get("tupian") %>' target='_blank'><img src='<%=map.get("tupian") %>' width=88 height=99 border=0 /></a></td>
 ...
 <td><%=map.get("jianchatixing") %></td>
	...
	<% if(map.get("jianchatixing").equals("是")){%>
	<a class="btn btn-danger btn" href="jiancha2.jsp?id=<%=map.get("id")%>"> <i class="glyphicon glyphicon-pencil" > </i> 完成检查</a> <%}%> 
	<a class="btn btn-warning" href="wodecheliang_list2.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')"> <i class="glyphicon glyphicon-wrench" > </i> 删除</a><a class="btn btn-danger btn" href="wodecheliang_updt.jsp?id=<%=map.get("id")%>"> <i class="glyphicon glyphicon-pencil" > </i> 修改</a>     <a class="btn btn-info btn-small" href="wodecheliang_detail.jsp?id=<%=map.get("id")%>"> <i class="glyphicon glyphicon-random" > </i> 详细</a> </td>
  </tr>
  	<%
  }
   %>
...
%>
 <style>
#winpop { width:400px; height:0;background:#FFFFFF; position:absolute;  right:0; bottom:0; border:1px solid #666; margin:0; padding:1px; overflow:hidden; display:none;} 
#winpop .title { width:100%; height:30px; line-height:30px; background:#E6E6E6; font-weight:bold; text-align:center; font-size:15px;} 
#winpop .con { width:100%; height:300px; line-height:20px; font-weight:bold; font-size:12px; color:#FF0000; text-align:left} 
...
<div class="con">当前有<%=zonglichengtx%>条检查提醒请尽快处理，谢谢！</div>
<%
}
%>
  </body>
