...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="cccccc" class="table table-striped table-bordered table-hover">  
 ...
 <% 
    ...
    String url = "fuwudingdan_list3.jsp?1=1"; 
    String sql =  "select * from fuwudingdan where yonghuming='"+request.getSession().getAttribute("username")+"' ";
...
if(request.getParameter("yuangongxingming")=="" ||request.getParameter("yuangongxingming")==null ){}else{sql=sql+" and yuangongxingming like '%"+request.getParameter("yuangongxingming")+"%'";}
 ...
 <td align='center'><%=map.get("iszf")%>  <% if(map.get("iszf").equals("否")){%><a class="btn btn-info btn-small" href="zhifu/index.jsp?id=<%=map.get("id")%>&biao=fuwudingdan"><font color='red'>支付</font></a> <%}%> </td>
    
    <td width="138" align="center"><%=map.get("addtime") %></td>
    <td width="220" align="center">    <a class="btn btn-info btn-small" href="fuwudingdan_detail.jsp?id=<%=map.get("id")%>"> <i class="glyphicon glyphicon-random" > </i> 详细</a> </td>
  </tr>
  	<%
  }
   %>
