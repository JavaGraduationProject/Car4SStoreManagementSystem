...
if(request.getParameter("zhuangtai")=="" ||request.getParameter("zhuangtai")==null ){}else{sql=sql+" and zhuangtai like '%"+request.getParameter("zhuangtai")+"%'";}
 ...
 <td align='center'><%=map.get("iszf")%>  <% if(map.get("iszf").equals("否")){%><a class="btn btn-info btn-small" href="zhifu/index.jsp?id=<%=map.get("id")%>&biao=dingdanxinxi&shifujine=<%=map.get("shifujine")%>&jifen=<%=request.getSession().getAttribute("jifen")%>"><font color='red'>支付</font></a> <%}%> </td>
    <td width="138" align="center"><%=map.get("addtime") %></td>
    <td width="220" align="center">     <a class="btn btn-info btn-small" href="dingdanxinxi_list2.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a> <a class="btn btn-info btn-small" href="dingdanxinxi_detail.jsp?id=<%=map.get("id")%>">详细</a> </td>
  </tr>
  	<%
  }
   %>
