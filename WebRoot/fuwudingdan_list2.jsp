...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="cccccc" class="table table-striped table-bordered table-hover">  
 ...
 <% 
    ...
    String url = "fuwudingdan_list2.jsp?1=1"; 
    String sql =  "select * from fuwudingdan where yuangonggonghao='"+request.getSession().getAttribute("username")+"' ";
...
if(request.getParameter("yuangongxingming")=="" ||request.getParameter("yuangongxingming")==null ){}else{sql=sql+" and yuangongxingming like '%"+request.getParameter("yuangongxingming")+"%'";}
 ...
 <td><a class="btn btn-info btn-small" href='<%=map.get("mingxibiao") %>' target='_blank'>下载</a></td>
 <td><%=map.get("zongfeiyong") %></td>
 
 <td align='center'><%=map.get("iszf")%>   </td>
    
    <td width="138" align="center"><%=map.get("addtime") %></td>
    <td width="220" align="center"><a class="btn btn-warning" href="fuwudingdan_list2.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')"> <i class="glyphicon glyphicon-wrench" > </i> 删除</a><a class="btn btn-danger btn" href="fuwudingdan_updt.jsp?id=<%=map.get("id")%>"> <i class="glyphicon glyphicon-pencil" > </i> 修改</a>     <a class="btn btn-info btn-small" href="fuwudingdan_detail.jsp?id=<%=map.get("id")%>"> <i class="glyphicon glyphicon-random" > </i> 详细</a> </td>
  </tr>
  	<%
  }
   %>
