...
<link rel="stylesheet" href="images/hsgcommon/div.css" type="text/css">
  ...
  <form name="form1" id="form1" method="post" action="">
   ...
   <input type="submit" name="Submit" value="查找" class="btn btn-info btn-small" /> <input type="button" name="Submit2" value="导出EXCEL" class="btn btn-info btn-small" onClick="javascript:location.href='shangpinxinxi_listxls.jsp';" />
</form>
...
if (request.getParameter("xiaoshoujiage2")==""  ||request.getParameter("xiaoshoujiage2")==null ) {}else {sql=sql+" and xiaoshoujiage <= '"+request.getParameter("xiaoshoujiage2")+"'";}
  ...
  <tr>
    ...
    <td align='center'><a class="btn btn-info btn-small" href="sh.jsp?id=<%=map.get("id")%>&yuan=<%=map.get("issh")%>&tablename=shangpinxinxi" onClick="return confirm('您确定要执行此操作？')"><%=map.get("issh")%></a></td>
    <td width='90' align='center'><a  href='<%=map.get("tupian") %>' target='_blank'><img src='<%=map.get("tupian") %>' width=88 height=99 border=0 /></a></td>
    
    
	
    <td width="90" align="center" bordercolor="#F3F3F3"><a class="btn btn-info btn-small" href="pinglun_list.jsp?id=<%=map.get("id")%>&biao=shangpinxinxi">评论管理</a></td>
    <td width="138" align="center"><%=map.get("addtime") %></td>
    <td width="220" align="center"><a class="btn btn-info btn-small" href="shangpinxinxi_updt.jsp?id=<%=map.get("id")%>">修改</a>  <a class="btn btn-info btn-small" href="shangpinxinxi_list.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a> <a class="btn btn-info btn-small" href="shangpinxinxidetail.jsp?id=<%=map.get("id")%>" target="_blank">详细</a> </td>
  </tr>
  	<%
  }
   %>
