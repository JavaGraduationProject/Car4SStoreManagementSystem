...
public class Info {
	...
	System.out.print(Info.getBetweenDayNumber("2012-11-11 11:19", "2012-11-11 11:11"));
	 ...
	 public static String getselect(String name,String tablename,String zdname)
	 {
	  ...
	  public static String getselectsl2(String name,String tablename,String zdname,String  fff)
	 {
		 String select = "<select name=\""+name+"\" id=\""+name+"\" onchange='gow();' class='form-control2'>";
		 select+="<option value=''>请选择</option>";
		
		 for(HashMap permap:new CommDAO().select("select * from "+tablename+"  where yonghuming='"+fff+"' order by id desc")){ 
			 select+="<option value=\""+permap.get(zdname)+"\">"+permap.get(zdname)+"</option>";
		 }
		 select+="</select>";
		 return select;
